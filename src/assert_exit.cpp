...
void cutb_assert_fail(int condition, const char *condstr, const char *filename,
                      int lineno)
{
...
void cutb_assert_eq_long_impl(long expected, long actual,
...
void cutb_assert_eq_ulong_impl(unsigned long expected, unsigned long actual,
                               const char *expectedstr, const char *actualstr,
                               const char *filename, int lineno)
{
    if (expected != actual) {
...
void cutb_assert_eq_dbl_impl(double expected, double actual, double tolerance,
                             const char *expectedstr, const char *actualstr,
                             const char *tolerancestr, const char *filename,
                             int lineno)
{
    if (fabs(expected - actual) > tolerance) {
        fprintf(stderr, "\n%s:%d: CUTB_ASSERT_EQ_DBL(%s, %s, %s) failed. "
                        "[fabs(%f - %f) > %f]\n",
                filename, lineno, expectedstr, actualstr, tolerancestr,
