...
#define CUTB_ASSERT(condition) \
    ((condition) ? (void)0 : cutb_assert_fail((condition), #condition, __FILE__, __LINE__))
#define CUTB_ASSERT_EQ_LONG(expected, actual) \
     cutb_assert_eq_long_impl(expected, actual, #expected, #actual, __FILE__, __LINE__)
#define CUTB_ASSERT_EQ_ULONG(expected, actual) \
     cutb_assert_eq_ulong_impl(expected, actual, #expected, #actual, __FILE__, __LINE__)
#define CUTB_ASSERT_EQ_DBL(expected, actual, tolerance) \
    cutb_assert_eq_dbl_impl(expected, actual, tolerance, #expected, #actual, #tolerance, __FILE__, __LINE__)
